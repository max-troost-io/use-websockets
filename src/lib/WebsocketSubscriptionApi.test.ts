...
describe('WebsocketSubscriptionApi', () => {
  ...
  describe('registerHook', () => {
    ...
    it('should warn when multiple hooks are registered', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('has more than one initiator'));
  ...
  describe('onMessage', () => {
    ...
    it('should call onMessage callback with data and uriApi', () => {
      ...
      expect(onMessageSpy).toHaveBeenCalledWith({ data: testData, uriApi: api });
    });
  ...
  describe('integration scenarios', () => {
    ...
    it('should handle disconnect then reconnect cycle', () => {
      ...
      // First connection
      api.onOpen();
      expect(sendSpy).toHaveBeenCalledWith(expect.objectContaining({ method: 'subscribe' }));
      ...
      // Reconnect
      api.onOpen();
      expect(sendSpy).toHaveBeenCalledWith(expect.objectContaining({ method: 'subscribe', body: { filter: 'active' } }));
    });
